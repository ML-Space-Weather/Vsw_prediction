...
def main():
    ...
    # Mark all occurrences of -9999.9 as True in column 'Nan_marked'
    ace['Nan_marked'] = ace['Speed'] == nan_label
    # Replace all occurrences of -9999.9 with NaN
    ace['Speed'].replace(nan_label, np.nan, inplace=True)
    # Interpolate the missing values
    ace['Speed'].interpolate(inplace=True)
    ...
    # List all files in a directory for cropped GONG data
    ...
    if Synchnization:
        for ii, filename in enumerate(tqdm(files)):
            ...
            try:
                ...
                # Extracting the 5days ahead data
                end_index = ace.index[pd.to_datetime(ace['Time'])> time_GONG + timedelta(days=5)][0]
                start_index = end_index - 654
                V = ace.loc[start_index:end_index+1, 'Speed'].values
                R = ace.loc[start_index:end_index+1, 'HC_radius'].values
                
                # st()
                
                new_V_5days.append(V[::-1])
                mean_R_5days.append(np.mean(R))

                # Extracting the current time data
                end_index = ace.index[pd.to_datetime(ace['Time']) >= time_GONG][0]
                start_index = end_index - 654
                V = ace.loc[start_index:end_index+1, 'Speed'].values
                R = ace.loc[start_index:end_index+1, 'HC_radius'].values
                # Roll the first 120 points to the end
                V = np.roll(V, -120)

                new_V.append(V[::-1])
                mean_R.append(np.mean(R))
                
                print('diff is {}'.format(new_V_5days[-1][121] - ace.loc[end_index:end_index, 'Speed'].values))
                st()
            
                # Extracting the GONG data
